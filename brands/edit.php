...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($name)) {
        $stmt = $conn->prepare("SELECT COUNT(*) as count FROM brands WHERE name = :name AND id != :id");
    ...
    if (empty($errors)) {
        try {
            ...
            $update_stmt->bindParam(':logo_url', $logo_url);
            $update_stmt->bindParam(':website', $website);
            $update_stmt->bindParam(':is_active', $is_active, PDO::PARAM_INT);
            $update_stmt->bindParam(':id', $brand_id);

            if ($update_stmt->execute()) {
                // Log the activity
                logActivity($conn, $user_id, 'brand_updated', "Updated brand: $name (ID: $brand_id)");
            ...
            $errors['general'] = 'An error occurred while updating the brand. Please try again.';
            error_log("Brand update error: " . $e->getMessage());
        }
    }
}

// Function for sanitizing brand input
...
<head>
    ...
    <title>Edit Brand - <?php echo htmlspecialchars($brand['name']); ?> - <?php echo htmlspecialchars($settings['company_name'] ?? 'POS System'); ?></title>
        ...
        <header class="header">
            <div class="header-content">
                <div class="header-title">
                    <h1>Edit Brand</h1>
                    <div class="header-subtitle">Update brand information for <?php echo htmlspecialchars($brand['name']); ?></div>
                </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control <?php echo isset($errors['name']) ? 'is-invalid' : ''; ?>"
                                       id="name" name="name" value="<?php echo htmlspecialchars($_POST['name'] ?? $brand['name']); ?>"
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active"
                                           value="1" <?php echo (isset($_POST['is_active']) ? $_POST['is_active'] : $brand['is_active']) ? 'checked' : ''; ?>>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="description" name="description" rows="4"
                                      placeholder="Describe the brand, its history, or special features"><?php echo htmlspecialchars($_POST['description'] ?? $brand['description'] ?? ''); ?></textarea>
                            <div class="form-text">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="url" class="form-control <?php echo isset($errors['website']) ? 'is-invalid' : ''; ?>"
                                       id="website" name="website" value="<?php echo htmlspecialchars($_POST['website'] ?? $brand['website'] ?? ''); ?>"
                            ...
                            <div class="form-group">
                                ...
                                <input type="url" class="form-control <?php echo isset($errors['logo_url']) ? 'is-invalid' : ''; ?>"
                                       id="logo_url" name="logo_url" value="<?php echo htmlspecialchars($_POST['logo_url'] ?? $brand['logo_url'] ?? ''); ?>"
