<?php
// This file is included by quotation.php when action=create
...
<head>
    ...
    <title>Create Quotation - <?php echo htmlspecialchars($settings['company_name'] ?? 'POS System'); ?></title>
...
<body>
        ...
        <div class="container-fluid" style="margin-bottom: 3rem;">
            <div class="quotation-form-container">
                <div class="form-header">
                    ...
                    <p class="mb-0">Follow the steps below to create a quotation</p>
                </div>
                ...
                <form id="quotationForm">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control" id="customer_id" name="customer_id">
                                        ...
                                        while ($customer = $customers->fetch(PDO::FETCH_ASSOC)) {
                                            echo "<option value=\"{$customer['id']}\" data-email=\"{$customer['email']}\" data-phone=\"{$customer['phone']}\">{$customer['name']}</option>";
                                        }
                                        ?>
                                    </select>
                    ...
                    <div class="form-step" id="step2">
                        <div class="items-section">
                            <h5 class="mb-3"><i class="bi bi-box"></i> Add Products to Quotation</h5>
                         ...
                         <div class="modal fade" id="barcodeScannerModal" tabindex="-1">
                            ...
                            <div id="itemsContainer">
                                <div class="text-center py-4">
                                    <p class="text-muted">Use the search products button or barcode scanner to add products to your quotation</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-step" id="step3">
                        <div class="totals-section">
                            ...
                            <div class="totals-grid">
                                <div>
                                    ...
                                    <input type="number" class="form-control total-input" id="subtotal" readonly>
                                </div>
                                <div>
                                    <label class="form-label">Tax Amount</label>
                                    <input type="number" class="form-control total-input" id="tax_amount" readonly>
                                </div>
                                <div>
                                    <label class="form-label">Final Total</label>
                                    <input type="number" class="form-control total-input fw-bold" id="final_amount" readonly>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="terms" name="terms" rows="3" placeholder="Terms and conditions for this quotation"><?php echo $settings['quotation_terms'] ?? ''; ?></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="date" class="form-control" id="valid_until" name="valid_until" 
                                           value="<?php echo date('Y-m-d', strtotime('+' . ($settings['quotation_valid_days'] ?? 30) . ' days')); ?>">
                        ...
                        <div class="text-center mt-4">
                            <button type="button" class="btn btn-save me-2" onclick="saveQuotation('draft')">
                                <i class="bi bi-save"></i> Save as Draft
                            </button>
                            <button type="button" class="btn btn-save" onclick="saveQuotation('sent')">
                                <i class="bi bi-send"></i> Save & Send
                            </button>
                            <a href="quotations.php" class="btn btn-secondary ms-2">
    ...
    <script>
        ...
        // Removed addItem() function - only inventory products allowed
        ...
        function saveQuotation(status) {
            ...
            // Save quotation with improved error handling
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 30000); // 30 second timeout for save operation

            fetch('quotation_save.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Cache-Control': 'no-cache'
            ...
            .then(data => {
                if (data.success) {
                    alert(`Quotation ${data.quotation_number} ${status === 'sent' ? 'created and sent' : 'saved as draft'} successfully!`);
                    window.location.href = `quotation.php?quotation_id=${data.quotation_id}`;
                } else {
        ...
        // Initialize calculations and load filters
        document.addEventListener('DOMContentLoaded', function() {
            calculateTotals();
            // Removed loadSearchFilters() from initial load for better performance
            // Filters will be loaded when product search modal is opened
            updateStepIndicators();
            updateNavigationButtons();
            updateStepInfo();
        });
